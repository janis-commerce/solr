...
class Filters {
	...
	static build(filters) {
		...
		return Object.values(filtersByType).reduce((formattedFilters, filter) => (formattedFilters ? `${formattedFilters} AND ${filter}` : filter), '');
	}
	...
	static _formatByType(filters) {
		...
		return Object.entries(filters).reduce((filtersByType, [field, filter]) => {
			...
			filtersByType[field] = formatter(field, filter.value || filter);
