...
class Schema {
	...
	static query(schema, currentSchemas) {
		...
		const fieldsToReplace = this._difference(builtSchema, fieldsToAdd)
			.filter(field => !currentSchemas.some(currentField => this._areEqualFields(currentField, field)));

		const fieldsToDelete = this._difference(currentSchemas, builtSchema)
			.map(({ name }) => ({ name }));

		if([fieldsToAdd, fieldsToReplace, fieldsToDelete].every(fieldsGroup => !fieldsGroup.length))
			return null;
	...
	static _areEqualFields(fieldA, fieldB) {
