...
class Solr {
	...
	async createCore(model, name) {
		...
		const endpoint = Endpoint.create('admin/cores?action=CREATE&name={{name}}&configSet=_default', this.url, null, { name });
	 ...
	 * @throws When something goes wrong
	 */
	async createSchema(model, core = this.core) {
		...
		const endpoint = Endpoint.create(Endpoint.presets.schema, this.url, core);
