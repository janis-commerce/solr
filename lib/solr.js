...
class Solr {
	...
	async updateSchema(model) {
		...
		const fieldTypes = model.constructor.fieldTypes || {};

		const currentSchemas = await this.getSchema();
		const currentFieldTypes = await this.getFieldTypes();

		const query = Schema.query(schema, currentSchemas, fieldTypes, currentFieldTypes);
	...
	/**
	 * Get the existing fields types from Solr
	 * @returns {Array.<object>} The actual fields types in Solr
	 * @throws When something goes wrong
	 */
	async getFieldTypes() {
		...
		const { fieldTypes } = res;

		return Schema.format(fieldTypes);
	}
	...
	async get(model, params = {}) {
		...
		const page = params.page || DEFAULT_START_PAGE;
