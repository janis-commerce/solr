...
class Solr {
	...
	async get(model, params = {}) {
		...
		const query = Query.build({ ...params, page, limit });
	...
	async updateSchemas(model) {
		...
		const { table } = model.constructor;
		const query = Schema.buildQuery('replace', model);
		const endpoint = Endpoint.create(Endpoint.presets.schemas, this._url, table);
	...
	_validateResponse(res) {
		...
		if(!responseHeader || responseHeader.status !== 0)
			throw new SolrError(`Invalid Solr response: ${res}`, SolrError.codes.INTERNAL_SOLR_ERROR);

		if(response && !response.docs)
			throw new SolrError(`Invalid Solr response: ${response}`, SolrError.codes.INTERNAL_SOLR_ERROR);
	}
