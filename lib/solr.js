...
class Solr {
	...
	async coreExists() {
		...
		try {
			...
			const res = await Request.post(endpoint, null, this.auth);

			Response.validate(res, { status: { [this.core]: { name: 'string' } } });
	 ...
	 * @param {boolean} shouldUpdateSchemas Specifies if must update fields schema after creating the core.
	 * Default: true
	 * @returns {Boolean} true if the operation was successful
	 * @throws When something goes wrong
	 */
	async createCore(model, shouldUpdateSchemas = true) {
		...
		return shouldUpdateSchemas ? this.updateSchema(model) : true;
	}
	...
	async getSchema() {
		...
		const res = await Request.get(endpoint, {}, this.auth);
	...
	async updateSchema(model) {
		...
		const res = await Request.post(endpoint, query, this.auth);
	...
	async insert(model, item) {
		...
		const res = await Request.post(endpoint, item, this.auth);
	...
	async multiInsert(model, items) {
		...
		const res = await Request.post(endpoint, items, this.auth);
	...
	async get(model, params = {}) {
		...
		const res = await Request.get(endpoint, query, this.auth);
	...
	async remove(model, item) {
		...
		const res = await Request.post(endpoint, { delete: { id: item.id } }, this.auth);
